...
import { Recycle, Leaf, ShieldCheck, MapPin, Monitor, Cpu, Smartphone, Battery, Printer, Server, CheckCircle2, Phone, Facebook, Youtube, Linkedin, Instagram, ChevronDown, PlayCircle, ArrowLeft, Users, Target, Globe, Award, Heart, Lightbulb } from 'lucide-react';
      ...
      <main className="max-w-7xl mx-auto px-4 py-20">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center mb-32">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-1 rounded-full bg-emerald-50 text-emerald-700 text-xs font-bold uppercase tracking-widest mb-8 border border-emerald-100">
              <Recycle className="w-4 h-4" /> Nossa História
            </div>
            <h1 className="text-5xl md:text-7xl font-light mb-8 serif italic text-emerald-950">Transformando o descarte em consciência.</h1>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center mb-32">
          ...
          <div className="order-1 lg:order-2">
            <h2 className="text-4xl md:text-5xl font-light text-emerald-950 mb-8 serif italic">Nossa Cultura</h2>
            <div className="space-y-8">
              {[
                { icon: <Lightbulb className="w-6 h-6" />, title: "Inovação Constante", desc: "Buscamos sempre novas formas de tornar a reciclagem mais eficiente e acessível." },
                { icon: <Heart className="w-6 h-6" />, title: "Paixão pelo Planeta", desc: "Cada ação nossa é movida pelo desejo de deixar um mundo melhor para as próximas gerações." },
                { icon: <Award className="w-6 h-6" />, title: "Excelência Técnica", desc: "Garantimos os mais altos padrões de segurança e destinação final homologada." }
              ].map((item, i) => (
                <div key={i} className="flex gap-6">
                  ...
                  <div>
                    <h4 className="text-xl font-medium text-emerald-900 mb-2">{item.title}</h4>
